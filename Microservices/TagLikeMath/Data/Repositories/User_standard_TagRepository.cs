...
namespace TagLikeMath.Data.Repositories
{
    internal class User_standard_TagRepository : IUser_standard_TagRepository
    {
        ...
        {
            return _context.User_standard_Tag.Where(t => t.user_id == user_id && t.tag_id == tag_id).FirstOrDefault();
        }
        public IQueryable<User_standard_Tag> GetAll()
        {
            return _context.User_standard_Tag.AsQueryable();
        }
        public IQueryable<User_standard_Tag> GetUser_standard_TagByUserId(int user_id)
        {
            return _context.User_standard_Tag.Where(t => t.user_id == user_id);
        }
        //-----------------------------------------
        public bool Add(User_standard_Tag newEntity)
        {
            var entity = _context.User_standard_Tag.Add(newEntity);
            return true;
        }
        //-----------------------------------------
        public void Remove(User_standard_Tag entity)
        {
            _context.User_standard_Tag.Remove(entity);
        }
        //-----------------------------------------
        public void Update(User_standard_Tag entity)
        {
            _context.User_standard_Tag.Update(entity);
        }
        //-----------------------------------------
