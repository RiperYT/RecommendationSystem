...
namespace TagLikeMath.Data.Repositories
{
    internal class Post_stat_TagRepository : IPost_stat_TagRepository
    {
        ...
        {
            return _context.Post_stat_Tag.Where(t => t.post_id == post_id && t.tag_id == tag_id).FirstOrDefault();
        }
        public IQueryable<Post_stat_Tag> GetAll()
        {
            return _context.Post_stat_Tag.AsQueryable();
        }
        public IQueryable<Post_stat_Tag> GetPost_stat_TagByPostId(int post_id)
        {
            return _context.Post_stat_Tag.Where(t => t.post_id == post_id);
        }
        //-----------------------------------------
        public bool Add(Post_stat_Tag newEntity)
        {
            var entity = _context.Post_stat_Tag.Add(newEntity);
            return true;
        }
        //-----------------------------------------
        public void Remove(Post_stat_Tag entity)
        {
            _context.Post_stat_Tag.Remove(entity);
        }
        //-----------------------------------------
